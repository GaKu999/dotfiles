...
if [[ "$PARENT_TERM" == foot* || "$TERM" == foot* ]]; then
	PS1="\[\e[96m\]\[\e[0m\] \[\e[1;96m\]\D{%Y-%m-%d}\[\e[0m\] \[\e[96m\]\[\e[0m\] \[\e[1;96m\]\D{%H:%M}\[\e[0m\] \[\e[93m\]\[\e[0m\] \[\e[1;93m\]\j\[\e[0m\]\n"
...
function unipua { local name="$1"
	case "$name" in
		bmp) printf "%b " $(printf "\\\u%x\n" {57344..63743}) ;;
		spua-a) printf "%b " $(printf "\\\U000f%x\n" {0..65535}) ;;
		spua-b) printf "%b " $(printf "\\\U0010%x\n" {0..65535}) ;;
		*) return 2 ;;
	esac
}
