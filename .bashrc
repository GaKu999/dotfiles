...
# man:less(1)
if [[ "$PARENT_TERM" == foot* || "$TERM" == foot* ]]; then
...
function aria2bt { local profile threads
	...
	: ${profile:=public}
	: ${threads:=$(nproc)}
	aria2c -j"$threads" --conf-path="$XDG_CONFIG_HOME/aria2bt/$profile.conf" --dht-file-path=dht.dat --dht-file-path6=dht6.dat --save-session=aria2bt.session --input-file=aria2bt.session
}
...
function ipls {
	...
	ipv4="$(drill -4DQ myip.opendns.com @resolver1.opendns.com A)"
	if [[ -n "$ipv4" ]]; then
		echo -e "\e[1mexternal:\e[0m \e[35m$ipv4\e[0m"
	fi
	ipv6="$(drill -6DQ myip.opendns.com @resolver1.opendns.com AAAA)"
	if [[ -n "$ipv6" ]]; then
