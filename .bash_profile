...
if [[ -z "$SSH_CLIENT" ]]; then
	...
	function _prepare_graphical_environment { local protocol="$1" desktop="$2"
		...
		if [[ -n "$desktop" ]]; then
			...
			export XDG_CURRENT_DESKTOP="${desktop^^}" XDG_MENU_PREFIX="$desktop-"
		else
			return 2
		fi
		unset -v PARENT_TERM
	...
	while true; do
		...
		case "$REPLY" in
			$'\012'|'s')
				...
				exec systemd-run --user -dqG -EWLR_BACKENDS=headless,libinput,drm --scope -usway-session --slice-inherit -- sway
			;;
			'c')
				echo -ne "\e[H\e[2J"
